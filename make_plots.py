...
#    'dilep_m[0]': " -b '15,76,106' -n dilep_m0_zoom", #low mass
#    'network_output_2020_11_19_test_checkpoint_6j_dcorr_lam10':  ' -b "4,0,1"',
#    'network_output_2020_11_19_test_checkpoint_6j_syst':  ' -b "4,0,1"',
...
cuts = {
#    'Zee_119prod':('n_jet>=4','(lep_pt[0]>27e3 && n_el==2 && n_baseel==2 && baseel_charge[0]!=baseel_charge[1] && Sum$(baseel_passECIDS==0)==0 && (el_trigger || mu_trigger) && lep_trigger_matched )*(sf_total*sf_mu_trigger*sf_el_trigger*sf_el_ECIDS)',"ee selection"),
#    'Zebasee_noambiguity_119prod':('n_jet>=4','(lep_pt[0]>27e3 && n_el>=1 && n_baseel==2 && baseel_charge[0]!=baseel_charge[1] && Sum$(baseel_passECIDS==0)==0 && Sum$(baseel_passSignalId && baseel_passSignalIso && abs(baseel_d0sig)<5 && baseel_pt>15e3)==2 && (el_trigger || mu_trigger) && lep_trigger_matched )*(sf_total*sf_mu_trigger*sf_el_trigger*sf_el_ECIDS)',"ee (no ambiguity cut) selection"),
#    'Zee_119prod_mll':('n_jet>=4','(lep_pt[0]>27e3 && n_el==2 && n_baseel==2 && (dilep_m[0]<101e3 && dilep_m[0]>81e3) && baseel_charge[0]!=baseel_charge[1] && Sum$(baseel_passECIDS==0)==0 && (el_trigger || mu_trigger) && lep_trigger_matched )*(sf_total*sf_mu_trigger*sf_el_trigger*sf_el_ECIDS)',"ee selection"),
#    'Zebasee_noambiguity_119prod_mll':('n_jet>=4','(lep_pt[0]>27e3 && n_el>=1 && n_baseel==2 && (dilep_m[0]<101e3 && dilep_m[0]>81e3) && baseel_charge[0]!=baseel_charge[1] && Sum$(baseel_passECIDS==0)==0 && Sum$(baseel_passSignalId && baseel_passSignalIso && abs(baseel_d0sig)<5 && baseel_pt>15e3)==2 && (el_trigger || mu_trigger) && lep_trigger_matched )*(sf_total*sf_mu_trigger*sf_el_trigger*sf_el_ECIDS)',"ee (no ambiguity cut) selection"),
...
#    '1L6j4b':('n_jet==6 && n_bjet>=4','(lep_pt[0]>27e3 && (el_trigger || mu_trigger) && lep_trigger_matched )*(sf_total*sf_mu_trigger*sf_el_trigger)',"Single lepton"),
...
for year in years:
  for cutname, (bothcut, rawcut, label) in cuts.iteritems():
    ...
    if "119prod" in cutname: tag = "_119prod"
    ...
    if "119prod" in cutname: opts += " --no-overlay --zoom-ratio"
